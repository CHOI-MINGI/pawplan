...
export async function generateDefaultCatCareSchedules(tx: Tx, catId: bigint, baseDate: Date, createdBy?: bigint) {
  const cat = await tx.cat.findUniqueOrThrow({ where: { id: catId } });
  const ageYears = cat.birthDate ? Math.max(0, (baseDate.getTime() - cat.birthDate.getTime()) / (365.25 * dayMs)) : 3;

  const templates = [
    {
      scheduleType: "heartworm",
      title: "심장사상충 예방",
      description: "고양이도 월 1회 심장사상충 예방이 권장됩니다.",
      dueDate: addDays(baseDate, 7),
      repeatCycleDays: 30,
      priority: "high",
    },
    ...
    {
      scheduleType: "vaccine",
      title: ageYears < 1 ? "기초 3종 접종 확인 (FVRCP)" : "고양이 3종 부스터 접종",
      description: ageYears < 1
        ? "범백혈구감소증·헤르페스바이러스·칼리시바이러스 기초 접종 이력을 확인하세요."
        : "FVRCP 연 1회 부스터 접종 일정입니다.",
      dueDate: addDays(baseDate, ageYears < 1 ? 14 : 60),
      repeatCycleDays: ageYears < 1 ? null : 365,
      priority: "high",
    },
    {
      scheduleType: "vaccine",
      title: "광견병 예방접종",
      description: "연 1회 권장되는 광견병 예방 일정입니다.",
      dueDate: addDays(baseDate, 75),
      repeatCycleDays: 365,
      priority: "high",
    },
    {
      scheduleType: "checkup",
      title: ageYears >= 8 ? "노령묘 건강검진" : "정기 건강검진",
      description: ageYears >= 8 ? "노령묘는 더 짧은 주기의 검진을 권장합니다." : "반기 또는 연 1회 검진 일정을 관리하세요.",
      dueDate: addDays(baseDate, ageYears >= 8 ? 45 : 90),
      repeatCycleDays: ageYears >= 8 ? 180 : 365,
      priority: ageYears >= 8 ? "high" : "medium",
    },
    {
      scheduleType: "grooming",
      title: "피부·위생 관리",
      description: "고양이 피부 상태와 발톱, 귀를 주기적으로 확인하세요.",
  ...
  const existing = await tx.catCareSchedule.count({ where: { catId, sourceType: "system" } });
  if (existing > 0) return 0;

  await tx.catCareSchedule.createMany({
    data: templates.map((template) => ({
      ...template,
      catId,
      createdBy,
